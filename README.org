...
| =anvil-shell-filter= (v0.4.0) | Shell output compression — 20 bundled filters for git / rg / pytest / ert-batch / docker-logs etc.  A bounded raw prefix is stashed with TTL for follow-up context; genuinely large output belongs in a file |
...
- Use `shell-run` for shell commands whose stdout can be filtered
  ...
  the bounded raw prefix with `shell-tee-get` only when the compressed
  view is insufficient. Redirect genuinely large output to a file.
